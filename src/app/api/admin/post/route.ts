import { NextRequest, NextResponse } from 'next/server';
import pool from '../../../../../lib/db'; // PostgreSQL connection
import fs from 'fs/promises'; // Use the promise-based version of fs
import path from 'path'; // For handling file paths
import { verifyToken } from '../../../../../lib/auth'; // Token verification function

export async function POST(req: NextRequest) {
  try {
    const token = req.headers.get('Authorization')?.split(' ')[1]; // Bearer token
    if (!token) {
      return NextResponse.json({ message: 'Authorization token missing' }, { status: 401 });
    }

    const adminId = await verifyToken(token);
    if (!adminId) {
      return NextResponse.json({ message: 'Invalid or expired token' }, { status: 401 });
    }

    const formData = await req.formData();
    const postHeader = formData.get('title')?.toString();
    const postMessage = formData.get('content')?.toString();
    const file = formData.get('image') as File | null;

    if (!postHeader || !postMessage) {
      return NextResponse.json({ message: 'Missing required fields' }, { status: 400 });
    }

    let imageUrl = '';
    if (file) {
      const validTypes = ['image/jpeg', 'image/png', 'image/gif'];
      if (!validTypes.includes(file.type)) {
        return NextResponse.json({ message: 'Invalid file type. Only JPEG, PNG, and GIF are allowed.' }, { status: 400 });
      }

      const uploadPath = path.join(process.cwd(), 'public/post', file.name);
      const arrayBuffer = await file.arrayBuffer();
      const buffer = Buffer.from(arrayBuffer);

      await fs.writeFile(uploadPath, buffer);
      imageUrl = `/post/${file.name}`;
    }

    const result = await pool.query(
      'INSERT INTO "Post" (title, content, "image", "username", "updatedAt") VALUES ($1, $2, $3, $4, $5)',
      [postHeader, postMessage, imageUrl, adminId, new Date()]
    );

    if (result.rowCount === 0) {
      return NextResponse.json({ message: 'Failed to create post' }, { status: 500 });
    }

    return NextResponse.json({
      message: 'Post created successfully!',
    }, { status: 201 });

  } catch (error) {
    console.error('Error during post creation:', error);
    return NextResponse.json({
      message: 'An unexpected error occurred',
      error: process.env.NODE_ENV === 'development' ? (error as Error).message : 'Internal Server Error',
    }, { status: 500 });
  }
}

// Handling GET requests to fetch posts along with user and post counts
export async function GET() {
  try {
    // Fetch posts
    const postResult = await pool.query('SELECT * FROM "Post" ORDER BY "createdAt" DESC');
    const posts = postResult.rows;

    // Fetch user count
    const userCountResult = await pool.query('SELECT COUNT(*) AS user_count FROM "User"');
    const userCount = userCountResult.rows[0].user_count;

    // Fetch post count
    const postCountResult = await pool.query('SELECT COUNT(*) AS post_count FROM "Post"');
    const postCount = postCountResult.rows[0].post_count;

    // Return both posts and counts in the response
    return NextResponse.json({
      posts,
      userCount,
      postCount,
    }, { status: 200 });

  } catch (error) {
    console.error('Error fetching data:', error);
    return NextResponse.json({
      message: 'An unexpected error occurred',
      error: process.env.NODE_ENV === 'development' ? (error as Error).message : 'Internal Server Error',
    }, { status: 500 });
  }
}








